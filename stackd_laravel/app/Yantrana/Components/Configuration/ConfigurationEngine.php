...
class ConfigurationEngine extends BaseEngine implements ConfigurationEngineInterface
{
    ...
    public function prepareConfigurations($pageType)
    {
        ...
        } else if ($pageType == 'promotion-plans') {
            $defaultPlanDuration = $defaultSettings['promotion_plan_duration']['default'];
            $dbPlanDuration = $configurationSettings['promotion_plan_duration'];
            $configurationSettings['promotion_plan_duration'] = combineArray($defaultPlanDuration, $dbPlanDuration);
    ...
    public function processConfigurationsStore($pageType, $inputData)
    {
        ...
        if ($pageType == 'premium-plans') {
            if (!__isEmpty($inputData['plan_duration'])) {
                foreach ($inputData['plan_duration'] as $m_key => $m_plan) {
                    foreach ($inputData['plan_duration'][$m_key] as $key => $plan) {
                        $inputData['plan_duration'][$m_key][$key] = [
