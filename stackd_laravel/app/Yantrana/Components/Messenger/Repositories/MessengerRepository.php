...
class MessengerRepository extends BaseRepository implements MessengerRepositoryInterface
{
    ...
    public function fetchUnreadConversations($userId)
    {
        $loggedInUserId = getUserID();
        return ChatModel::where('chats.users__id', $userId)
            ->where('chats.to_users__id', $loggedInUserId)
            ->Where('chats.from_users__id', $userId)
            ->Where('chats.status', '<>' ,3)
            ->whereNotIn('chats.type', [9, 10, 11])
            ->select(
                \__nestedKeyValues([
                    'chats' => [
    ...
    public function fetchCountMessageToday($userId) {
        return ChatModel::where('from_users__id', $userId)
        ->whereNotIn('type', [9, 10, 11])
        ->whereDate('created_at', Carbon::today())
        ->count();
    }

    public function fetchIsFollowedUser($userId) {
        return ChatModel::where('from_users__id', $userId)
        ->whereNotIn('type', [9, 10, 11])
        ->whereDate('created_at', Carbon::today())
        ->count();
    ...
    public function fetchLastReceivedMessage($userId)
    {
        $loggedInUserId = getUserID();
        return ChatModel::where('users__id', $loggedInUserId)
            ->where('from_users__id', $userId)
            ->where('to_users__id', $loggedInUserId)
            //->whereNotIn('type', [9, 10, 11])
            ->latest()
            ->first();
    }

    public function fetchUnreadMessageCount($userId)
    {
        $loggedInUserId = getUserID();
        return ChatModel::where('users__id', $userId)
            ->where('from_users__id', $userId)
            ->where('to_users__id', $loggedInUserId)
            ->whereNotIn('type', [9, 10, 11])
            ->where('status','<>', 3)     // 3 is seen of status
            ->count();
    }
