...
class MediaEngine extends BaseEngine implements MediaEngineInterface
{
     ...
     * @return array
     *---------------------------------------------------------------- */
    public function processSaveBase64ImageToLocalServer($base64String, $allowedExtension = '', $uid, $type)
    {

        // Check if base64String __isEmpty or is valid
        if (__isEmpty($base64String)) {
            return $this->engineReaction(2, null, __tr('invalid uploaded file.'));
        }
        ...
        file_put_contents($path.'/'.$fileName, file_get_contents($base64String));  // custom function with curl --- url_get_contents
        ...
        if($type == "photos"){
            $profileImageFolderPath = getPathByKey('user_photos', ['{_uid}' => $uid]);
        }
     ...
     * @return array
     *---------------------------------------------------------------- */
    public function resizeImageAndUpload($destinationPath, $fileName, $options = [], $uid = '')
    {
        $path = getPathByKey('user_temp_uploads', ['{_uid}' => authUID()]);
        if(!__isEmpty($uid)) {
            $path = getPathByKey('user_temp_uploads', ['{_uid}' => $uid]);
    ...
    public function uploadUserPhotos($inputFile, $requestFor)
    {
        ...
        if ($uploadedFileOnLocalServer['reaction_code'] == 1) {
            ...
            return $this->resizeImageAndUpload($userPhotosFolderPath, $fileName, [
                'height' => 312, // origin 820
                'width' => 312   // origin 312
            ]);
