...
class ManageUserEngine extends BaseEngine
{
    ...
    public function userSubscriptionsDataTableList()
    {
        $userCollection = $this->manageUserRepository->fetchUserSubscriptions();
        ...
        $requireColumns = [
            ...
            //'created',
            'profile_image' => function ($key) {
                $path = getPathByKey('profile_photo', ['{_uid}' => $key['userUId']]);
                return getMediaUrl($path, $key['profile_picture']);
            },
            'created' => function ($key) {
                return formatDate($key['created'], "l jS F Y g:i A");
            },
            'profile_url' => function ($key) {
                return route('user.profile_view', ['username' => $key['username']]);
            },
            "deleteUrl" => function ($key) {
                return route('manage.user.write.subscription_delete', [
                    'userUid' => $key['userUId'],
                    'subscriptionUid' => $key['_uid']
                ]);
            }
        ];
    ...
    public function processUserSubscriptionDelete($userUid, $subscriptionUid)
    {
        $transactionResponse = $this->manageUserRepository->processTransaction(function () use ($userUid, $subscriptionUid) {
            ...
            $userSubscription = $this->manageUserRepository->getUsersSubscription($userDetails->_id, $subscriptionUid);

            //if deleted 
            if ($this->manageUserRepository->deleteUserSubscription($userSubscription)) {
                return $this->manageUserRepository->transactionResponse(1, ['show_message' => true], __tr('Subscription removed successfully.'));
            }
