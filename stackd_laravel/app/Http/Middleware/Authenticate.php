...
class Authenticate
{
    ...
    public function handle($request, Closure $next)
    {
        ...
        if (isset($user) and !__isEmpty($user)) {
            ...
            if ($userAuthority->user_roles__id != 1) { // not admin
                ...
                if (!isProfileComplete($user->_id)) {
                    ...
                    if (!in_array($currentRouteName, [
                        ...
                        "user.sign_up_profile.process",
                        "user.sign_up_availability.process",
                        "user.sign_up_location.process",
                        "user.sign_up_location.process",
                        "user.write.setting-subscription"
                    ])) {
                        if ($request->ajax()) {
                            return __apiResponse([
                    ...
                    if( $userAuthority->user_roles__id == 3 ) { // user role is Pt
                        if( !isPremiumUserStripe($user->_id) ){
                            if (!in_array($currentRouteName, [
                                "user.update_profile.wizard",
                                "user.upload_cover_image",
                                "user.upload_profile_image",
