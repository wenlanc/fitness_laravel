...
<form class="lw-ajax-form lw-form" method="post" action="<?= route('manage.configuration.write', ['pageType' => request()->pageType]) ?>">
	<!-- premium plan container -->
		@foreach($configurationData['plan_duration'] as $plan_key => $plan_data)
			<div class="form-group mt-2">
				<fieldset class="lw-fieldset mb-3">
					...
					<div class="row">
						@foreach($configurationData['plan_duration'][$plan_key] as $key => $plan)
						<div class="col-sm-6">
							<div class="custom-control custom-checkbox">
								<!-- enable premium plans input hidden field -->
								<input type="hidden" name="plan_duration[<?= $plan_key ?>][<?= $key ?>][enable]" id="lwEnablePlan_<?= $plan_key.'_'.$key ?>" value="false" />
								<!-- /enable premium plans input hidden field -->
								<input type="checkbox" class="custom-control-input" id="lwEnable_<?= $plan_key.'_'.$key ?>" name="plan_duration[<?= $plan_key ?>][<?= $key ?>][enable]" value="true" <?= $plan['enable'] == 'true' ? 'checked' : '' ?>>
							...
							<div class="mb-3" id="lwPlanPriceInputField_<?= $plan_key.'_'.$key ?>">
								...
								<input type="number" class="form-control form-control-user" value="<?= $plan['price'] ?>" name="plan_duration[<?= $plan_key ?>][<?= $key ?>][price]" placeholder="<?= __tr('Price') ?>" id="lwPrice_<?= $plan_key.'_'.$key ?>">
							</div>
							<!-- / Plan Price -->
						</div>
						@endforeach
