...
<div class="pb-2 row row-cols-sm-1 row-cols-md-2 row-cols-lg-4" style="position:relative;">
    <h5 style="color: #FF0000;"> 
        PTs 
    </h5>
    @include('filter.search_filter')
</div>
<div class="row row-cols-sm-1 row-cols-md-1 row-cols-lg-1" id="lwPTFilterContainer" style="margin-top:10px;justify-content: center;">
    @include('filter.ajax.pt_list')
</div>
...
<script>
    function loadMoreUsers(responseData) {
        ...
        $('#lwPTFilterContainer .lw-load-more-container').remove();    
    ...
    function requestFilter(){
        ...
        var requestUrl = '<?= route('user.read.pt_list'); ?>';
        // post ajax request
        __DataRequest.get(requestUrl, formData, function(response) {
            if (response.reaction == 1) {
