...
$incognito_mode_setting = 1;
if(!isAdmin())
    $incognito_mode_setting = $featurePlanCollection[$user_role_name]['see_who_views_your_page']['select_user'];    


?>
@if( ($incognito_mode_setting==2 && !( $userMembership == 'pro' || $userMembership == 'premium')))

    <div class="d-flex mt-2 mb-2 visitor_premium_required_container"> 
        <div> 
            <div class="visitor_premium_required_title"> 
            There are <?= count($filterData)?> people that viewed your account recently. 
            </div>
            <div class="visitor_premium_required_subtitle"> 
            Gain accesses to who has visited your profile, and many more features.
            </div>
        </div>
        <div class="visitor_premium_required_btn_container">
            <a href="<?= route('user.read.setting', ['pageType' => 'billing']) ?>" class="btn visitor_premium_required_btn"> Join Stackd Pro </a>
        </div>
    </div>

    @if(!__isEmpty($filterData))
        @foreach($filterData as $filter)
            <div class="d-flex mt-2" style="border-radius:15px;" >
                <img src="/media-storage/pt/fuzzy_match.png" style="width:100%;height:100%;border-radius:15px;">
            </div>
        @endforeach
    @else
    <!-- info message -->
    <div class="col-sm-12 alert alert-info">
        <?= __tr('There are no visitor found.'); ?>
    </div>
    <!-- / info message -->
    @endif
@else 
    @if(!__isEmpty($filterData))
        @foreach($filterData as $filter)
        <div class="row row-cols-sm-1 row-cols-md-3 row-cols-lg-3" id="lwMatchFilterContainer" style="justify-content: center;">
            ...
            <div class="col mb-2 p-3" style="max-width:36.33%;">
                <div class="">
                    ...
                    <div class="row">
                        @if(0)
                        <span class="pt_follower_count"> <?= $filter["totalUserLike"] ?> Followers </span>
                        @endif
                    ...
                    <div class="row mt-2">
                        <span class="pt_usernickname" style="font-family: Nunito Sans;font-style: normal;font-weight: bold;font-size: 17px;line-height: 21px;"> 
                            ...
                            <span class="ml-2"><?= $filter['city']; ?></span>
            ...
            <div class="col mb-2 p-3">
                ...
                <div class="row nav-item dropdown no-arrow">
                    <!-- <span> <i style="font-size: 24px;color:white;content: url(/dist/blackfit/images/svg/chat.svg);width: 24px;height: 24px;margin-top: 2px;" class="far fa-comments"></i> </span> -->
                    ...
                    <a class="nav-link dropdown-toggle d-flex" href="#" role="button" data-toggle="dropdown" aria-haspopup="true" aria-expanded="false" style="padding-top:0px;">
...
@if($hasMorePages)
<div class="lw-load-more-container" id="lw-load-more-container-visitors">
    <button type="button" class="btn btn-light btn-block lw-ajax-link-action lw-load-more-btn" id="lwLoadMoreButtonVisitors" data-action="<?= $nextPageUrl ?>" data-callback="loadVisitors"><?= __tr('Load More') ?></button>
</div>
@endif